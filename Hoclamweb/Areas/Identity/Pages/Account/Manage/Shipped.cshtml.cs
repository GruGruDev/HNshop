...
namespace Suitshop.Areas.Identity.Pages.Account.Manage
{
	public class ShippedModel : PageModel
	{
		...
		{
			Orders = await _unitOfWork.Order.GetRange(x => x.ApplicationUserId == user.Id && x.OrderStatus == SD.Order_Shipped)
				.ToListAsync();

			Items = await _unitOfWork.Item.GetRange(x => x.Order.ApplicationUserId == user.Id && x.Order.OrderStatus == SD.Order_Shipped)
